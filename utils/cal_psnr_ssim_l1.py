...
def calculate_metrics(folder1, folder2):
    ...
    sorted_l1 = sorted(dl1_l)
    
    sorted_l1_idx = sorted(range(len(dl1_l)),key = lambda k: dl1_l[k], reverse= False)
    for i in range(len(dl1_l)):
        print(img_list[sorted_l1_idx[i]], sorted_l1[i])
    return psnr, ssim, dl1


if __name__ == '__main__':
    folder1 = 'data1k/result/tim_added/tmp/grid_mean'
    folder2 = 'data1k/test/full_224/'

    # fpath1 = os.path.join(folder1,'66.png')
    # fpath2 = os.path.join(folder2,'66.png')
    # img1 = read_image(fpath1).astype(np.float64)
    # img2 = read_image(fpath2).astype(np.float64)
    # print(calculate_l1(img1, img2))
