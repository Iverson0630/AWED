...
class Decoder(nn.Module):
    ...
    def forward(self,output):
        ...
        for idx,(unsample,conv,layers) in enumerate(reversed(self.stages)):
            if idx>2:
                return x
            x = unsample(x)
...
class Segformer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if self.decoder_type=='transblock':
            res = self.to_segmentation(self.decoder(layer_outputs))
        elif self.decoder_type=='mlp':
            fused = [to_fused(output) for output, to_fused in zip(layer_outputs, self.decoder)]
            fused = torch.cat(fused, dim = 1)
            res = self.to_segmentation(fused)
        return res
